...
public class FriendListFragment extends Fragment implements FriendAddDialogFragment.FriendAddDialogListener {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        new ItemTouchHelper(new ItemTouchHelper.SimpleCallback(0,
            ...
            public void onSwiped(@NonNull RecyclerView.ViewHolder viewHolder, int direction) {
                List<Friend> myFriends = MainActivity.myself.getFriends();
                Friend deleteFriend = friendAdapter.getFriendAt(viewHolder.getAdapterPosition
                        ());
                Friend friendToDelete = new Friend(0, "", new ArrayList<Friend>(), new ArrayList<Present>());
                for(Friend myF: myFriends) {
                    if(myF.getId() == deleteFriend.getId()) {
                        friendToDelete = myF;
                    }
                }
                myFriends.remove(friendToDelete);
                MainActivity.myself.setFriends(myFriends);
                friendViewModel.delete(MainActivity.myself);
    ...
    @Override
    public void onDialogPositiveClick(Friend friend) {
        List<Friend> myFriends = MainActivity.myself.getFriends();
        myFriends.add(friend);
        MainActivity.myself.setFriends(myFriends);
        friendViewModel.update(MainActivity.myself);
        friendAdapter.notifyDataSetChanged();
    }

    @Override
    public void onDialogNegativeClick(Friend friend) {
